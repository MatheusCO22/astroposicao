...
def get_dataframe():
    ...
    df = pd.DataFrame(columns=['Number', 'Name', 'Identifier', 'Constellation', 'VMagnitude', 'AMagnitude', 'Distance', 'RAJ2000', 'DECJ2000'])
    ...
    for row in table.tbody.find_all('tr'):    
        ...
        if(columns != []):
            ...
            VMagnitude = columns[4].text.strip()
            AMagnitude = columns[7].text.strip()
            ...
            df = df._append({'Number': number, 'Name': name, 'Identifier': identifier,
                             ...
                             'VMagnitude': VMagnitude, 'AMagnitude': AMagnitude,
                             'Distance': distance,
    ...
    df = df.astype({"Number": int, "Name": str, "Identifier": str,
                    "Constellation": str,
                    "VMagnitude": float, "AMagnitude": float,
                    "Distance": float,
